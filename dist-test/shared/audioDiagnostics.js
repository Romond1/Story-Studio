...
exports.deriveAudioEngineState = deriveAudioEngineState;
exports.deriveAudioWarnings = deriveAudioWarnings;
function deriveAudioEngineState(input) {
    if (input.reconnecting)
        return "reconnecting";
    if (input.warnings.some((warning) => warning.severity === "error"))
        return "error";
...
function deriveAudioWarnings(state) {
    const warnings = [];
    const add = (id, severity, label, detail) => warnings.push({ id, severity, label, detail });
    if (state.selectedDevicesMissing.length > 0) {
        add("missing-device", "error", "Saved audio device is missing", `Reconnect or replace: ${state.selectedDevicesMissing.join(", ")}.`);
    }
    if (state.masterFlowing && !state.mixFlowing) {
        add("mix-output-silent", "error", "Master audio is present but Mix Output is silent", "Test or reconnect the Mix/Virtual Output route.");
    }
    if (state.masterFlowing && !state.monitorFlowing) {
        add("monitor-output-silent", "warning", "Master audio is present but Monitor Output is silent", "Check monitor mute, monitoring state, and the selected device.");
    }
    ...
    if (state.mixPhysicalKey
        && state.monitorPhysicalKey
        && state.mixPhysicalKey === state.monitorPhysicalKey) {
        add("same-physical-output", "warning", "Monitor and Mix resolve to the same output", "Choose separate devices to avoid delayed or doubled lesson audio.");
    }
